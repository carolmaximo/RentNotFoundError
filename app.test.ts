...
import { RentNotFoundError } from "./rentNotFoundError";
...
describe('App', () => {
    ...
    it('should throw RentNotFoundError when returning an unregistered rent', () => {
    const user = new User('Maria', 'maria@mail.com', '1234');
    const bike = new Bike('mountain bike', 'mountain', 123, 500, 100.5, 'desc', 5, [], 'bike1');
    const today = new Date();
    const twoDaysFromToday = new Date();
    twoDaysFromToday.setDate(twoDaysFromToday.getDate() + 2);
    app.rentBike(user, bike, today, twoDaysFromToday);
    const returnedDate = new Date();
    returnedDate.setDate(returnedDate.getDate() + 1);
    expect(() => app.returnBike(new Rent(bike, user, new Date(), new Date()), returnedDate)).toThrow(RentNotFoundError);
});